...
make_random_init <- function(n_rows, n_cols, p_alive, seed=NULL) {
...
compute_hxc <- function(x, emb) {
  op_probs <- op_pd(x, D = emb, tau=1)
  
  probs <- op_probs$probabilities
  h_raw <- permutation_entropy(probs) # raw entropy, up to ln(D!)
  h <- h_raw / log(factorial(emb))    # normalize to [0,1]
  c <- statistical_complexity(probs)
  
  c(H = h, C = c)
}

compute_hxc_old <- function(x, emb) {
...
results_CAsimulation <- ca_history5k_0.8

dir.create("frames_5k_0-8", showWarnings = FALSE)

for (t in seq_along(results_CAsimulation)) {
  mat <- results_CAsimulation[[t]]
  img <- array(1 - mat, dim = c(nrow(mat), ncol(mat), 1))
  writePNG(img, target = sprintf("frames_5k_0-8/frame_%04d.png", t - 1))
}

# Debuging

mat <- ca_history10k_0.8[[1]]
