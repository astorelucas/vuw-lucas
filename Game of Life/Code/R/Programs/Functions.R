...
run_gol_hxc_analysis_selfcontained <- function(seed, k, T, emb, window_size = 4, stride = 4) {
...
run_gol_hxc_analysis_from_simulation <- function(maps, emb, window_size, stride, seed) {
...
run_gol_hxc_analysis_from_simulation_pht <- function(maps, emb, group_size = 16, seed) {
...
plot_time_series_epoch_faceted <- function(result, hc_df, epoch, n_panels = 4) {
  ...
  ggplot(df, aes(x = index, y = value)) +
    ...
    labs(title = sprintf("Time series - Epoch %d", epoch),
         x = "Window index (Hilbert order)", y = "binary_to_real value") +
...
analyze_convergence <- function(hc_df) {
  ...
  if (any(is.na(df$epoch))) {
    warning("Some epoch values could not be coerced to numeric — check hc_df$epoch")
  }
  ...
  p_velocity <- ggplot(df, aes(x = epoch, y = velocity)) +
    ...
    geom_point(color = "steelblue", size = 0.8, alpha = 0.6) +
    scale_y_log10() +
    labs(title = "Convergence velocity over epochs",
         x = "Epoch", y = "Velocity (log scale)") +
    theme_minimal(base_size = 12) +
  ...
  p_dist <- ggplot(df, aes(x = epoch, y = dist_to_limit)) +
    ...
    labs(title = "Distance to H×C boundary over epochs",
         x = "Epoch", y = "dist_to_limit (log scale)") +
    theme_minimal(base_size = 12) +
    theme(plot.title = element_text(face = "bold"),
          panel.grid.minor = element_blank())
  ...
  p_dist <- p_dist +
    geom_line(data = df, aes(y = dist_fit), color = "black",
              linetype = "dashed", linewidth = 0.5)
  
  df$vel_fit <- exp(predict(fit_vel))
  p_velocity <- p_velocity +
    geom_line(data = df, aes(y = vel_fit), color = "black",
              linetype = "dashed", linewidth = 0.5)
  
  combined <- p_velocity / p_dist +
...
diagnose_convergence_residuals <- function(hc_df, fit_dist, fit_velocity,outlier_z = 2) {
  ...
  p1 <- ggplot(df, aes(x = epoch, y = resid_dist)) +
    ...
    labs(title = "Residuals: log(dist_to_limit) ~ epoch",
         x = "Epoch", y = "Residual", color = "Outlier") +
    theme_minimal(base_size = 12) +
  ...
  p2 <- ggplot(df, aes(x = epoch, y = resid_vel)) +
    ...
    scale_color_manual(values = c("FALSE" = "darkorange", "TRUE" = "firebrick")) +
    labs(title = "Residuals: log(velocity) ~ epoch",
         x = "Epoch", y = "Residual", color = "Outlier") +
    theme_minimal(base_size = 12) +
    theme(plot.title = element_text(face = "bold"), legend.position = "none")
  ...
  p3 <- ggplot(df, aes(x = epoch, y = dist_to_limit)) +
    ...
    geom_point(data = filter(df, outlier_dist & !outlier_vel),
               aes(y = dist_to_limit), color = "steelblue", size = 1.5) +
    geom_point(data = filter(df, outlier_vel & !outlier_dist),
               aes(y = dist_to_limit), color = "darkorange", size = 1.5) +
    geom_point(data = filter(df, outlier_both),
               aes(y = dist_to_limit), color = "firebrick", size = 2) +
    labs(title = "dist_to_limit trajectory with flagged reorganization epochs",
  ...
  cat("overlap rate (of dist outliers):",
      round(100 * n_both / max(n_dist_outliers, 1), 1), "%\n")
  cat("overlap rate (of velocity outliers):",
  ...
  list(
    df       = df,
    plot     = combined,
    outliers_both = filter(df, outlier_both) %>% select(epoch, H, C, velocity, dist_to_limit)
  )
}
...
analyze_ccf_residuals <- function(diag_df, max_lag = 20) {
  ...
  gaps <- diff(df$epoch)
  if (any(gaps != 1)) {
    warning("epoch não é sequencial/regular — CCF pode não ser interpretável diretamente. ",
  ...
  ccf_result <- ccf(resid_vel_ts, resid_dist_ts, lag.max = max_lag, plot = FALSE)
  ...
  p_ccf <- ggplot(ccf_df, aes(x = lag, y = acf)) +
    ...
    geom_segment(aes(xend = lag, yend = 0), color = "darkorange", linewidth = 0.6) +
    geom_point(color = "darkorange", size = 1.5) +
    labs(title = "Cross-correlation: velocity residuals vs dist_to_limit residuals",
         subtitle = "Positive lag: velocity leads dist_to_limit | dashed lines = 95% significance",
         x = "Lag (epochs)", y = "Cross-correlation") +
    theme_minimal(base_size = 12) +
    theme(plot.title = element_text(face = "bold"),
          plot.subtitle = element_text(size = 9, color = "grey40"))
  ...
  if (nrow(sig_lags) > 0) {
    ...
    cat("  lag > 0: velocity residual at t predicts dist_to_limit residual at t+lag\n")
    cat("  lag < 0: dist_to_limit residual at t predicts velocity residual at t+|lag|\n")
  } else {
    cat("Nenhum lag ultrapassou o limite de significância — não há evidência de\n")
...
plot_dist_to_limit <- function(hc_df, fit_dist = NULL, log_scale = FALSE) {
  ...
  if (!is.null(fit_dist)) {
    ...
    p <- p +
      geom_ribbon(data = df, aes(y = fit, ymin = fit_lo, ymax = fit_hi),
                  fill = "firebrick", alpha = 0.15, color = NA) +
      geom_line(data = df, aes(y = fit),
...
# ---------------------------------------------------------------
find_tau_stationary <- function(hc_df, tau_grid = seq(300, 1500, by = 100), alpha = 0.05, vars = c("H", "C")) {
...
fit_decay_for_tau <- function(hc_df, tau) {
  ...
  list(tau = tau, r = r, half_life = half_life, fit = fit, df = df,
       H_star = asym$H_star, C_star = asym$C_star)
}
# ---------------------------------------------------------------
